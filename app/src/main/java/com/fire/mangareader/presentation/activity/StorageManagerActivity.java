    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        btnDeleteRead.setOnClickListener(v -> {
            new Thread(() -> {
                AppDatabase db = AppDatabase.getInstance(this);
                List<DownloadedChapter> downloads = db.downloadDao().getAllDownloads();
                List<com.fire.mangareader.data.database.ChapterState> readStates = db.chapterStateDao().getAllReadStates();
                
                int count = 0;
                for (DownloadedChapter chapter : downloads) {
                    boolean isRead = false;
                    for (com.fire.mangareader.data.database.ChapterState state : readStates) {
