...
public class MangaDetailActivity extends AppCompatActivity {
        ...
        if (btnStartReading != null) {
            btnStartReading.setOnClickListener(v -> {
                if (chapterList != null && !chapterList.isEmpty()) {
                    ...
                    com.fire.mangareader.domain.model.Chapter firstChapter = chapterList.get(chapterList.size() - 1);
    ...
    private void checkRoomCacheAndLoad() {
        ...
        new Thread(() -> {
            com.fire.mangareader.data.database.CachedManga cachedManga = AppDatabase.getInstance(this).cacheDao().getMangaDetails(mangaUrl);
            ...
            runOnUiThread(() -> {
                if (cachedManga != null && cachedChapters != null && !cachedChapters.isEmpty()) {
                    ...
                    chapterList.clear();
                    for (com.fire.mangareader.data.database.CachedChapter cc : cachedChapters) {
    ...
    private void saveToRoomCache(String desc, String status, List<Chapter> chapters) {
        new Thread(() -> {
            com.fire.mangareader.data.database.CachedManga cm = new com.fire.mangareader.data.database.CachedManga();
            cm.mangaUrl = mangaUrl;
            cm.description = desc;
            cm.status = status;

            List<com.fire.mangareader.data.database.CachedChapter> ccList = new ArrayList<>();
            for (int i = 0; i < chapters.size(); i++) {
                Chapter c = chapters.get(i);
                com.fire.mangareader.data.database.CachedChapter cc = new com.fire.mangareader.data.database.CachedChapter();
    ...
    private void checkFavoriteStatus() {
        ...
        com.fire.mangareader.data.network.SupabaseManager.getInstance(this).checkLibraryStatus(mangaUrl, new com.fire.mangareader.data.network.SupabaseManager.DataCallback() {
            @Override
    ...
    private void loadReadChapters() {
        new Thread(() -> {
            List<com.fire.mangareader.data.database.ChapterState> states = AppDatabase.getInstance(this).chapterStateDao().getAllStatesForManga(mangaUrl);
            ...
            for (com.fire.mangareader.data.database.ChapterState state : states) {
    ...
    private void loadAniListMetadata() {
        ...
        com.fire.mangareader.util.AniListManager.fetchMetadata(mangaTitle, new com.fire.mangareader.util.AniListManager.AniListCallback() {
            @Override
            public void onSuccess(com.fire.mangareader.domain.model.AniListMetadata metadata) {
                runOnUiThread(() -> {
    ...
    private void showMultiCriteriaRatingDialog() {
        ...
        builder.setPositiveButton("إرسال التقييم", (dialog, which) -> {
            ...
            com.fire.mangareader.util.GlobalMangaStatsManager.submitRating(MangaDetailActivity.this, mangaUrl, mangaTitle, overall, story, characters, art, new com.fire.mangareader.util.GlobalMangaStatsManager.RatingCallback() {
                @Override
        ...
        targetView.animate().scaleX(0.9f).scaleY(0.9f).setDuration(100).withEndAction(() -> {
            ...
            new androidx.appcompat.app.AlertDialog.Builder(MangaDetailActivity.this)
                    ...
                    .setSingleChoiceItems(options, checkedItem, (dialog, which) -> {
                        ...
                        if (which == 4) {
                            // Remove
                            com.fire.mangareader.data.network.SupabaseManager.getInstance(MangaDetailActivity.this).removeFromLibrary(mangaUrl, new com.fire.mangareader.data.network.SupabaseManager.AuthCallback() {
                                @Override
