...
public class ReplyAdapter extends RecyclerView.Adapter<ReplyAdapter.ViewHolder> {
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        ...
        holder.btnLike.setOnClickListener(v -> {
            if (reply.id != null) {
                com.fire.mangareader.network.SupabaseManager.getInstance(context)
                    .likeComment(reply.id, reply.likes + 1, new com.fire.mangareader.network.SupabaseManager.AuthCallback() {
                        @Override
        ...
        holder.btnMore.setOnClickListener(v -> {
            if (reply.user_id != null && reply.user_id.equals(com.fire.mangareader.network.SupabaseManager.getInstance(context).getCurrentUserId())) {
                new android.app.AlertDialog.Builder(context)
                    .setTitle("حذف الرد")
                    .setMessage("هل أنت متأكد من حذف هذا الرد؟")
                    .setPositiveButton("حذف", (dialog, which) -> {
                        com.fire.mangareader.network.SupabaseManager.getInstance(context).deleteComment(reply.id, new com.fire.mangareader.network.SupabaseManager.AuthCallback() {
                            @Override
                            public void onSuccess(String message) {
                                ((android.app.Activity) context).runOnUiThread(() -> {
